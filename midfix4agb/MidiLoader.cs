...
namespace FileMIDI
{
    static class MidiLoader
    {
            ...
            for (int currentTrack = 0; currentTrack < numTracks; currentTrack++)
            {
                ...
                while (true)
                {
                    ...
                    if (eventTypeByte == 0xFF)      // if META Event
                    {
                        ...
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, metaType, metaData, true));
                    ...
                    else if (eventTypeByte == 0xF0 || eventTypeByte == 0xF7)        // if SysEx Event
                    {
                        ...
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, eventTypeByte, sysexData, false));
                    }
                    else if (eventTypeByte >> 4 == 0x8)     // if Note OFF command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = midiBinaryStream.ReadByte();
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteOFF));
                    ...
                    else if (eventTypeByte >> 4 == 0x9)     // if Note ON command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = midiBinaryStream.ReadByte();
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteON));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 == 0xA)     // if Aftertouch command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = midiBinaryStream.ReadByte();
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteAftertouch));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 == 0xB)     // if MIDI controller command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = midiBinaryStream.ReadByte();
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.Controller));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 == 0xC)     // if Preset command
                    {
                        ...
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.Program));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 == 0xD)     // if Channel Aftertouch command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = 0x0;    // unused
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.ChannelAftertouch));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 == 0xE)     // if Pitch Bend command
                    {
                        byte par1 = midiBinaryStream.ReadByte();
                        byte par2 = midiBinaryStream.ReadByte();
                        midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.PitchBend));
                        // save the last event type and channel
                    ...
                    else if (eventTypeByte >> 4 < 0x8)
                    {
                        ...
                        switch (lastEventType)
                        {
                            case NormalType.NoteOFF:
                                ...
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteOFF));
                                break;
                            case NormalType.NoteON:
                                par2 = midiBinaryStream.ReadByte();
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteON));
                                break;
                            case NormalType.NoteAftertouch:
                                par2 = midiBinaryStream.ReadByte();
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteAftertouch));
                                break;
                            case NormalType.Controller:
                                par2 = midiBinaryStream.ReadByte();
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.Controller));
                                break;
                            case NormalType.Program:
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, 0x0, NormalType.Program));
                                break;
                            case NormalType.ChannelAftertouch:
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, 0x0, NormalType.ChannelAftertouch));
                                break;
                            case NormalType.PitchBend:
                                par2 = midiBinaryStream.ReadByte();
                                midiTracks[currentTrack].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.PitchBend));
                                break;
                        }
                    }
                    else
        ...
        private static void loadAndConvertTypeZero(string filePath, ref List<MidiTrack> midiTracks, ref ushort timeDivision)    // returns the MIDI loaded in the List of all individual MIDI channels split up into 16 tracks
        {
            ...
            while (true)
            {
                ...
                if (eventTypeByte == 0xFF)      // if META Event
                {
                    ...
                    midiTracks[0].MidiEvents.Add(new MidiEvent(currentTick, metaType, metaData, true));
                }
                else if (eventTypeByte == 0xF0 || eventTypeByte == 0xF7)        // if SysEx Event
                {
                    long sysexLength = readVariableLengthValue(ref midiBinaryStream);
                    byte[] sysexData = new byte[sysexLength];
                    midiBinaryStream.Read(sysexData, 0, (int)sysexLength);
                    midiTracks[0].MidiEvents.Add(new MidiEvent(currentTick, eventTypeByte, sysexData, false));
                }
                else if (eventTypeByte >> 4 == 0x8)     // if Note OFF command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = midiBinaryStream.ReadByte();
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteOFF));
                ...
                else if (eventTypeByte >> 4 == 0x9)     // if Note ON command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = midiBinaryStream.ReadByte();
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteON));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.NoteON;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 == 0xA)     // if Aftertouch command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = midiBinaryStream.ReadByte();
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.NoteAftertouch));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.NoteAftertouch;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 == 0xB)     // if MIDI controller command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = midiBinaryStream.ReadByte();
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.Controller));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.Controller;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 == 0xC)     // if Preset command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = 0x0;
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.Program));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.Program;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 == 0xD)     // if Channel Aftertouch command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = 0x0;
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.ChannelAftertouch));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.ChannelAftertouch;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 == 0xE)     // if Pitch Bend command
                {
                    byte par1 = midiBinaryStream.ReadByte();
                    byte par2 = midiBinaryStream.ReadByte();
                    midiTracks[eventTypeByte & 0xF].MidiEvents.Add(new MidiEvent(currentTick, (byte)(eventTypeByte & 0xF), par1, par2, NormalType.PitchBend));
                    // now backup channel and Normal Type for truncated commands
                    lastEventType = NormalType.PitchBend;
                    lastMidiChannel = (byte)(eventTypeByte & 0xF);
                }
                else if (eventTypeByte >> 4 < 0x8)
                {
                    ...
                    switch (lastEventType)
                    {
                        case NormalType.NoteOFF:
                            par2 = midiBinaryStream.ReadByte();
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteOFF));
                            break;
                        case NormalType.NoteON:
                            par2 = midiBinaryStream.ReadByte();
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteON));
                            break;
                        case NormalType.NoteAftertouch:
                            par2 = midiBinaryStream.ReadByte();
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.NoteAftertouch));
                            break;
                        case NormalType.Controller:
                            par2 = midiBinaryStream.ReadByte();
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.Controller));
                            break;
                        case NormalType.Program:
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, 0x0, NormalType.Program));
                            break;
                        case NormalType.ChannelAftertouch:
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, 0x0, NormalType.ChannelAftertouch));
                            break;
                        case NormalType.PitchBend:
                            par2 = midiBinaryStream.ReadByte();
                            midiTracks[lastMidiChannel].MidiEvents.Add(new MidiEvent(currentTick, lastMidiChannel, par1, par2, NormalType.PitchBend));
                            break;
                    }
                }
                else
        ...
        internal static byte[] ushortToBigEndian(ushort value)   // returns a byte Array in Big Endian of an ushort ; FINISHED
        {
        ...
        internal static byte[] intToBigEndian(int value)         // returns a byte Array in Big Endian of an int ; FINISHED
        {
        ...
        internal static ushort loadBigEndianUshort(byte[] dataValues)    // returns an ushort by the Big Endian values in the Array ; FINISHED
        {
            return (ushort)(dataValues[0] << 8 | dataValues[1]);
        }

        internal static int loadBigEndianInt(byte[] dataValues)       // returns an int by the Big Endian values in the Array ; FINISHED
        {
            return dataValues[0] << 24 | dataValues[1] << 16 | dataValues[2] << 8 | dataValues[3];
        }

        internal static long readVariableLengthValue(ref BinaryReader midiBinaryStream)     // reads a variable Length value from the Filestream at its current position and extends the Stream position by the exact amount of bytes ; FINSHED
        {
